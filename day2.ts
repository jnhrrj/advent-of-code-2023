...
// Conclusion: Learned about loop labels. Should probably never use them again tho.
// Maybe solve by chaining methods like any/some/all next time
...
for (let game of inputRows) {
  ...
  setsLoop: for (let set of sets) {
    ...
    for (let cubeDraw of cubeDraws) {
      cubeDraw = cubeDraw.trim();
      const [num, color] = cubeDraw.split(" ");
      const allowedMax = bagConfiguration[color as CubeColor];
      if (parseInt(num) > allowedMax) {
        id = 0;
        break setsLoop;
      }
    }
...
// Conclusion: How was this the first time I've ever used reduce. Love it. Will use again
...
for (let game of inputRows) {
  ...
  for (let set of sets) {
    ...
    for (let cubeDraw of cubeDraws) {
      cubeDraw = cubeDraw.trim();
      const [num, color] = cubeDraw.split(" ");
      if (minNumOfColors[color as CubeColor] < parseInt(num))
        minNumOfColors[color as CubeColor] = parseInt(num);
    }
  }
  setPowerSum += Object.values(minNumOfColors).reduce(
    (acc, value) => acc * value
  );
}
