...
// Conclusion: unnecessary recursion and generally bad approach.
// Probably should've went the other way around and checked each number first
// and if it is near a "part" instead of checking if parts have numbers around them
// Edit: That was before I read part II. I take it all back
...
const recursiveRowSearch = (
  ...
  if (!searchedPos.includes(`${leftX}-${y}`)) {
    searchedPos.push(`${leftX}-${y}`);
    const leftNeighbor = inputRows[y][leftX];
    if (isNumeric(leftNeighbor)) {
      currentDigits.unshift(leftNeighbor);
      recursiveRowSearch([leftX, y], searchedPos, currentDigits);
    }
  }

  const rightX = x + 1;

  if (!searchedPos.includes(`${rightX}-${y}`)) {
    searchedPos.push(`${rightX}-${y}`);
    const rightNeighbor = inputRows[y][rightX];
    if (isNumeric(rightNeighbor)) {
      currentDigits.push(rightNeighbor);
      recursiveRowSearch([rightX, y], searchedPos, currentDigits);
    }
  }
};

for (let [y, row] of inputRows.entries()) {
  for (let [x, char] of Array.from(row).entries()) {
    // True when char represents an engine part
    if (!isNumeric(char) && char !== ".") {
      const searchedPos: string[] = [];
      const neighborNumbers: number[] = [];

      directionOffsets.forEach(([dx, dy]) => {
        const newX = x + dx;
        const newY = y + dy;
        if (searchedPos.includes(`${newX}-${newY}`)) return;
        searchedPos.push(`${newX}-${newY}`);

        const neighborChar = inputRows[newY][newX];

        if (neighborChar !== undefined && isNumeric(neighborChar)) {
          const currentDigits = [neighborChar];
          recursiveRowSearch([newX, newY], searchedPos, currentDigits);
          const num = parseInt(currentDigits.join(""));
          neighborNumbers.push(num);
        }
      });

      partSum += neighborNumbers.reduce((acc, number) => acc + number);
    }
  }
}
